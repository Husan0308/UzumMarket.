...
export default function Like() {
    ...
    const [alertData, setAlertData] = useState({ name: '', image: 'http://UzumMarket12.pythonanywhere.com' });  
    ...
    const handleAddToCart = async (item) => {
        ...
        try {
            ...
            const response = await axios.post('http://UzumMarket12.pythonanywhere.com/api/add_cart/', { id: item.id }, {
                headers: {
            ...
            setAlertData({ 
                ...
                name: item.title, 
                img: `http://UzumMarket12.pythonanywhere.com${item.image}` 
            });
            setShowAlert(true);
    ...
    useEffect(() => {
        (async () => {
            try {
                const accessToken = localStorage.getItem('accessToken');
                const response = await axios.get('http://UzumMarket12.pythonanywhere.com/api/get_cart_items/', {
                    headers: {
                        'Authorization': `Bearer ${accessToken}`
                    }
                });
    ...
    useEffect(() => {
        const fetchLikedItems = async () => {
            try {
                const accessToken = localStorage.getItem('accessToken');
                const response = await axios.get('http://UzumMarket12.pythonanywhere.com/api/get_liked/', {
                    headers: {
                        'Authorization': `Bearer ${accessToken}`
                    }
                });
    ...
    const removeFromCart = async (itemId) => {
        try {
            ...
            const accessToken = localStorage.getItem('accessToken');
            const response = await axios.delete(`http://UzumMarket12.pythonanywhere.com/api/delete_liked/${itemId}/`, {
                headers: {
                    'Authorization': `Bearer ${accessToken}`
                }
            });
                    ...
                    {liked.map(item => (
                        <div key={item.id} className="card">
                            <div className="card-image">
                                ...
                                <img 
                                    src={`http://UzumMarket12.pythonanywhere.com/${item.image}/`} 
