...
export default function Like() {
    ...
    const [alertData, setAlertData] = useState({ name: '', image: 'http://127.0.0.1:8000' });  
    ...
    const handleAddToCart = async (item) => {
        ...
        try {
            ...
            const response = await axios.post('http://127.0.0.1:8000/api/add_cart/', { id: item.id }, {
                headers: {
    ...
    useEffect(() => {
        const fetchLikedItems = async () => {
            try {
                const accessToken = localStorage.getItem('accessToken');
                const response = await axios.get('http://127.0.0.1:8000/api/get_liked/', {
                    headers: {
                        'Authorization': `Bearer ${accessToken}`
                    }
                });
    ...
    const removeFromCart = async (itemId) => {
        try {
            ...
            const accessToken = localStorage.getItem('accessToken');
            const response = await axios.delete(`http://127.0.0.1:8000/api/delete_liked/${itemId}/`, {
                headers: {
                    'Authorization': `Bearer ${accessToken}`
                }
            });
